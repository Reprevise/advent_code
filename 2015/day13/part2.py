...
def do(me=True):
    with open('C:\\Users\\thesl\\Desktop\\advent_code\\2015\\day13\\input.txt') as f:
        strings = [s.strip() for s in f.readlines()]
    people = set()
    happiness = dict()
    for s in strings:
        a, pos_neg, happy_units, b = re.match(
            r"(\w+) would (\w+) (\d+) happiness units by sitting next to (\w+)", s).groups()
        people.update([a, b])
