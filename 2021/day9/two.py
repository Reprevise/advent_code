...
def check_basin(matrix: list[list[int]], x: int, y: int, visited_nodes: list[tuple[int, int]], r_len: int, c_len: int):
...
for heights in heights_sum:
    i, j, z = heights
    visited = []
    check_basin(heightmap, i, j, visited, len(
        heightmap) - 1, len(heightmap[0]) - 1)
